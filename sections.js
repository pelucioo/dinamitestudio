(() => {
  ...
  let current=Math.max(0,panels.findIndex(p=>'#'+p.id===location.hash)),busy=false,wheelSum=0,lastWheel=0,lastDirection=0,wheelReadyAt=0;
  ...
  const show=async(index,updateURL=true,focus=false)=>{
    ...
    const options={duration:reduced.matches?0:520,easing:'cubic-bezier(.25,.1,.25,1)',fill:'both'};
  ...
  stage.addEventListener('wheel',e=>{
    if(e.ctrlKey||document.querySelector('dialog[open]')||Math.abs(e.deltaX)>Math.abs(e.deltaY)||!e.deltaY)return;
    const direction=Math.sign(e.deltaY),now=performance.now(),gap=now-lastWheel;
    if(gap>180||direction!==lastDirection)wheelSum=0;
    lastWheel=now;lastDirection=direction;
    if(busy){e.preventDefault();wheelSum=0;return;}
    if(canScroll(e.target,direction)){wheelSum=0;return;}
    e.preventDefault();
    // Rearm after each transition, without requiring the user to stop scrolling.
    // Discard residual trackpad movement instead of carrying it into another slide.
    const delta=e.deltaY*(e.deltaMode===1?16:e.deltaMode===2?stage.clientHeight:1);
    if(now<wheelReadyAt||Math.abs(delta)<4){wheelSum=0;return;}
    wheelSum+=delta;
    if(Math.abs(wheelSum)>=48){wheelSum=0;show(current+direction);}
  },{passive:false});
